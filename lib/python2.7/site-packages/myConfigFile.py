...
class MyCfgFile(object):
    ...
    def setChkInterval(self, interval):
        self.__logger.info("Set job checking interval (minitus) to %s", interval)
        self.__setOpt('base', 'JobCheckInterval', interval)
    
    def getChkInterval(self):
        return self.__cfgObj.get('base', 'JobCheckInterval')
    ...
    def getPrjDB(self, prjName):
        if self.__cfgObj.has_option('project', prjName):
            ...
            return self.__cfgObj.get('project', prjName)
        else:
            self.__logger.error("There is no such project in config file: %s", prjName)
            return ''
    
    def getPrjList(self):
        return self.__cfgObj.options('project')
    ...
    def __setOpt(self, section, optName, optValue):
        self.__cfgObj.set(section, optName, str(optValue))
        self.__update()
