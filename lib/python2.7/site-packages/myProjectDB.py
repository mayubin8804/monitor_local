...
class MyProjectDB(object):
    def __init__(self, prjName, prjDBFile):
        ...
        self.__prjDBFile = prjDBFile
        self.__logger.info("Open project db: %s", self.__prjDBFile)
        self.__storage = FileStorage.FileStorage(self.__prjDBFile)
        self.__db = DB(self.__storage)
        self.__conn = self.__db.open()
        self.__dbroot = self.__conn.root()
        if not self.__dbroot.has_key('projectName'):
            self.__dbroot['projectName'] = prjName
            self.__dbroot['maxJobNum']   = 10
            self.__dbroot['currentJobs'] = 0
            self.__dbroot['allFinishedFlag'] = False
            self.__dbroot['jobs'] = OOBTree()
    
    def setMaxJobNum(self, maxJobNum):
        self.__dbroot['maxJobNum'] = maxJobNum
        ...
        with open(jobListFile, 'r') as fh:
            for line in fh:
                ...
                for jobStr in jobStrList:
                    ...
                    self.__addJob2DB(job, jobObj)
                if secondJob != '':
                    self.__getJobObj(secondJob).addDependent(firstJob)
        self.__logger.info("Finish importing tasks. Total job number: %d, added job number: %d", totalJobNum, addJobNum)
        self.__close()
    
    def __close(self):
        self.__logger.info("Close project db: %s", self.__prjDBFile)
        transaction.commit()
        self.__conn.close()
        self.__db.close()
        self.__storage.close()
    
    def __addJob2DB(self, jobName, jobObj):
        self.__dbroot['jobs'][jobName] = jobObj
    
    def __hasJob(self, jobName):
        if self.__dbroot['jobs'].has_key(jobName):
            return True
        else:
            return False
    
    def __getJobObj(self, jobName):
        return self.__dbroot['jobs'][jobName]
    
    def __getDBJobList(self):
        return self.__dbroot['jobs'].keys()
...
def removeDBFile(db):
    dbFileList = [db, db + '.index', db + '.lock', db + '.tmp']
